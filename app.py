...
# Path to store flight data history (Render uses ephemeral storage, so this will reset on redeploy)
...
def handle_rockblock():
    data_json = request.get_json()  # Expect JSON payload from the test script
    imei = data_json.get('imei')
    data = data_json.get('data')

    print(f"Received POST /rockblock - IMEI: {imei}, Data: {data}")

    if imei != "301434061119410":  # Match professor's IMEI
    ...
    try:
        ...
        for x in range(5, 12):  # Pressure, 3 temps, Euler angles (div by 10)
            sensor_data[x] /= 10
        ...
        # Extract extra message text if present
        extra_message = ""
        if len(byte_data) > 50:
...
def animation_data():
    telemetry_data = {
        ...
        "position": {"x": random.uniform(-5, 5), "y": random.uniform(-5, 5), "z": random.uniform(-5, 5)},
        "force": {"x": random.uniform(0, 10), "y": random.uniform(0, 10), "z": random.uniform(0, 10)}
    }
